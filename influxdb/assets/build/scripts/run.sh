...
if [ "${USE_HTTPS}" == "true" ]; then
    ...
    # ADD ADMIN SSL certificates
    if [ ! -f /etc/influxdb/ssl/admin.pem ]; then
        mkdir -p /etc/influxdb/ssl && chmod 750 /etc/influxdb/ssl
        openssl req -x509 -newkey rsa:2048 -keyout /etc/influxdb/ssl/key.pem \
                -out /etc/influxdb/ssl/cert.pem -days ${VALIDITY} -subj "${ADMIN_DNAME}" -nodes
    	cat /etc/influxdb/ssl/key.pem /etc/influxdb/ssl/cert.pem > /etc/influxdb/ssl/admin.pem
    	rm  /etc/influxdb/ssl/key.pem /etc/influxdb/ssl/cert.pem
        chmod 0640 /etc/influxdb/ssl/admin.pem
    fi

    # ADD INFLUXDB SSL certificates
    if [ ! -f /etc/influxdb/ssl/influxdb.pem ]; then
        mkdir -p /etc/influxdb/ssl && chmod 750 /etc/influxdb/ssl
        openssl req -x509 -newkey rsa:2048 -keyout /etc/influxdb/ssl/key.pem \
                -out /etc/influxdb/ssl/cert.pem -days ${VALIDITY} -subj "${DB_DNAME}" -nodes
    	cat /etc/influxdb/ssl/key.pem /etc/influxdb/ssl/cert.pem > /etc/influxdb/ssl/influxdb.pem
    	rm  /etc/influxdb/ssl/key.pem /etc/influxdb/ssl/cert.pem
        chmod 0640 /etc/influxdb/ssl/influxdb.pem
    fi
else
